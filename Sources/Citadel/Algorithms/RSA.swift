...
//            return EncryptedMessage(rawRepresentation: result.serialize())
    ...
    public final class PrivateKey: NIOSSHPrivateKeyProtocol {
        ...
        public func signature<D: DataProtocol>(for message: D) throws -> Signature {
            ...
            // Use the correct hash algorithm based on the generic Hash type
            let hashBytes: [UInt8]
            let nid: Int32
            let hashLength: UInt32

            switch Hash.self {
            case is Insecure.SHA1.Type:
                hashBytes = Array(Insecure.SHA1.hash(data: message))
                nid = NID_sha1
                hashLength = 20
            case is SHA256.Type:
                hashBytes = Array(SHA256.hash(data: message))
                nid = NID_sha256
                hashLength = 32
            case is SHA512.Type:
                hashBytes = Array(SHA512.hash(data: message))
                nid = NID_sha512
                hashLength = 64
            default:
                // Fallback to SHA1 for unknown types
                hashBytes = Array(Insecure.SHA1.hash(data: message))
                nid = NID_sha1
                hashLength = 20
            }
